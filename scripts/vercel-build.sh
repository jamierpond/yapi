...
OS=$(uname -s | tr '[:upper:]' '[:lower:]')
if [ "$OS" = "darwin" ]; then
    # macOS - install to GOPATH/bin (same as Makefile)
    ...
    # Linux (Vercel) - /usr/local/bin is guaranteed on PATH
...
# Copy yapi binary - keep in bin/ which is at repo root
echo "yapi binary at ./bin/yapi"
chmod +x ./bin/yapi
ls -la ./bin/yapi
