...
function getWebviewContent(body, stderr, isLoading, executionTime) {
...
function getOrCreatePanel(context) {
    ...
    panel = vscode.window.createWebviewPanel('yapiResponse', 'RESPONSE', { viewColumn: vscode.ViewColumn.Beside, preserveFocus: true }, { enableScripts: false });
    panel.onDidDispose(() => {
        panel = undefined;
    }, null, context.subscriptions);
    return panel;
}
function validateYapiDocument(document) {
    ...
    validationTimeout = setTimeout(() => {
        ...
        try {
            ...
            let parsed = {};
            try {
                parsed = (__webpack_require__(3).load)(text);
            }
            catch (e) {
                const errorLine = e.mark?.line || 0;
                const diag = new vscode.Diagnostic(new vscode.Range(errorLine, 0, errorLine, lines[errorLine]?.length || 0), `YAML parse error: ${e.message}`, vscode.DiagnosticSeverity.Error);
                diagnostics.push(diag);
                diagnosticCollection.set(document.uri, diagnostics);
                return;
            }
            if (!parsed || typeof parsed !== 'object') {
                const diag = new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), 'Invalid YAML structure', vscode.DiagnosticSeverity.Error);
                diagnostics.push(diag);
            }
            else {
                if (!parsed.url) {
                    ...
                    const diag = new vscode.Diagnostic(new vscode.Range(line, 0, line, lines[line]?.length || 0), 'Missing required field: url', vscode.DiagnosticSeverity.Error);
                    diagnostics.push(diag);
                }
                ...
                if (hasBody && hasJson) {
                    ...
                    const diag = new vscode.Diagnostic(new vscode.Range(line, 0, line, lines[line]?.length || 0), 'Cannot specify both "body" and "json" fields', vscode.DiagnosticSeverity.Error);
                    diagnostics.push(diag);
                }
                if ((hasBody || hasJson) && !parsed.content_type && !parsed.url?.startsWith('grpc')) {
                    ...
                    const diag = new vscode.Diagnostic(new vscode.Range(line, 0, line, lines[line]?.length || 0), 'content_type is required when body or json is present', vscode.DiagnosticSeverity.Warning);
                    diagnostics.push(diag);
                }
                if (parsed.url?.startsWith('grpc://') || parsed.url?.startsWith('grpcs://')) {
                    if (!parsed.service) {
                        ...
                        const diag = new vscode.Diagnostic(new vscode.Range(line, 0, line, lines[line]?.length || 0), 'service field is required for gRPC requests', vscode.DiagnosticSeverity.Error);
                        diagnostics.push(diag);
                    }
                    if (!parsed.rpc) {
                        const rpcLine = lines.findIndex(l => l.trim().startsWith('rpc:'));
                        const line = rpcLine >= 0 ? rpcLine : 0;
                        const diag = new vscode.Diagnostic(new vscode.Range(line, 0, line, lines[line]?.length || 0), 'rpc field is required for gRPC requests', vscode.DiagnosticSeverity.Error);
                        diagnostics.push(diag);
                    }
                }
            }
            ...
            const diag = new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), `Validation error: ${error}`, vscode.DiagnosticSeverity.Error);
            diagnostics.push(diag);
        }
        diagnosticCollection.set(document.uri, diagnostics);
    }, 300);
}
async function runYapi(context) {
...
async function insertExample(exampleKey) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showErrorMessage('No active editor');
        return;
...
module.exports.types = {
  ...
  null:      __webpack_require__(17),
  pairs:     __webpack_require__(25),
  set:       __webpack_require__(26),
  timestamp: __webpack_require__(21),
  bool:      __webpack_require__(18),
  int:       __webpack_require__(19),
  merge:     __webpack_require__(22),
  omap:      __webpack_require__(24),
  seq:       __webpack_require__(15),
...
var makeSnippet         = __webpack_require__(7);
var DEFAULT_SCHEMA      = __webpack_require__(8);
